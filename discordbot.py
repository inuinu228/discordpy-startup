...
@bot.command()
async def fortfurimuki(ctx, c: float, d: float):
    await ctx.send('お前の振り向きは %.1f cmや \n わかったか？' % (round(2.54*180/(c*0.55550*d),1)))
    
@bot.command()
async def apexfurimuki(ctx, c: float, d: float):
    await ctx.send('お前の振り向きは %.1f cmや \n わかったか？' % (round(2.54*180/(c*0.022*d),1)))
...
async def help(ctx):
    ...
    embed.add_field(name="/fortfurimuki", value="DPIと感度を教えると俺がフォートナイトでの振り向きを測ってやるぞ", inline=False)
    embed.add_field(name="/apexfurimuki", value="DPIと感度を教えると俺がApexでの振り向きを測ってやるぞ", inline=False)
...
async def takayama(ctx):
    ...
    embed.add_field(name="DPI", value="1200", inline=False)
    embed.add_field(name="Mouse Sensitivity X", value="0.085", inline=False)
    embed.add_field(name="Mouse Sensitivity Y", value="0.085", inline=False)
    ...
    embed.add_field(name="Mousepad", value="G440", inline=False)
...
async def tokimatsu(ctx):
    ...
    embed.add_field(name="Mouse Sensitivity X", value="0.049", inline=False)
    embed.add_field(name="Mouse Sensitivity Y", value="0.049", inline=False)
...
async def haruki(ctx):
    ...
    embed.add_field(name="Mouse Targeting Sensitivity", value="0.30", inline=False)
    embed.add_field(name="Mouse Scope Sensitivity", value="0.30", inline=False)
